    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-xl mx-auto px-4 py-6 flex flex-col gap-5">
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
          <h2 className="font-semibold text-gray-800 mb-4">Ajukan Pencairan</h2>
          {hasPending ? (
            <div className="flex items-center gap-3 bg-yellow-50 border border-yellow-200 rounded-xl p-4 text-yellow-700 text-sm">
              <Clock className="h-5 w-5 flex-shrink-0" />
              <span>Permintaan pencairan sebelumnya sedang diproses admin. Tunggu hingga selesai.</span>
            </div>
          ) : saldo <= 0 ? (
            <div className="text-center py-4 text-gray-400 text-sm">Saldo belum cukup untuk dicairkan.</div>
          ) : (
        ...
        {history.length > 0 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-50">
              <h2 className="font-semibold text-gray-800">Riwayat Pencairan</h2>
            </div>
                ...
                return (
                  <div key={p.id} className="px-5 py-4">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-bold text-gray-800">{formatRupiah(p.jumlah)}</span>
                      <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full ${cfg.color}`}>
                        <Icon className="h-3.5 w-3.5" />{cfg.label}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500">{p.nama_bank} · {p.no_rekening} · {p.nama_pemilik}</p>
